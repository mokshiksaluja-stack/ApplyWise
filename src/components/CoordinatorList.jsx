...
const CoordinatorList = ({ coordinators }) => {
    ...
    <div className="bg-white p-5 rounded-[14px] shadow-[0_2px_8px_-2px_rgba(0,0,0,0.04)] border border-gray-200/60 flex flex-col h-full">
      ...
      <div className="flex flex-col gap-3 flex-1 justify-center">
        {coordinators.map((coordinator) => (
          <div key={coordinator.id} className="flex items-center group cursor-pointer hover:bg-gray-50/80 p-1.5 -mx-1.5 rounded-[10px] transition-colors">
            <img 
              src={coordinator.avatar} 
              alt={coordinator.name} 
              className="w-8 h-8 rounded-full object-cover shadow-[0_1px_3px_0_rgba(0,0,0,0.1)] border border-white mr-3"
            />
            <div className="flex-1">
              <h4 className="text-[13px] font-bold text-gray-800 group-hover:text-gray-900">{coordinator.name}</h4>
