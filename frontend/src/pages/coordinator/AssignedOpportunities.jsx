...
import { Search, MapPin, Users, Calendar, ArrowRight, Activity, Clock, SlidersHorizontal, Info } from 'lucide-react';
...
export default function AssignedOpportunities() {
  ...
  const { sharedOpportunities, currentCoordinatorId } = useAdminCoordinatorContext();
  
  const assignedDrives = sharedOpportunities.filter(opp => opp.assignedCoordinatorId === currentCoordinatorId);
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <div className="space-y-4">
        {assignedDrives.length === 0 ? (
          ...
          <div key={drive.id} className="bg-white rounded-2xl border border-gray-200 shadow-sm hover:shadow-md transition-all overflow-hidden p-5 flex flex-col md:flex-row gap-5 md:items-center">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 md:gap-8 flex-1 md:flex-none my-2 md:my-0">
              ...
              <div className="flex flex-col">
                ...
                <span className="text-sm font-bold text-gray-900">{drive.deadline || drive.scheduledDate}</span>
              </div>
              <div className="flex flex-col border-l border-gray-100 pl-4 lg:border-none lg:pl-0">
            ...
            <div className="w-full md:w-auto pt-4 md:pt-0 border-t border-gray-100 md:border-none md:pl-6">
              <button 
                onClick={() => navigate(`/coordinator/opportunities/${drive.id}`)}
